...
import InventoryHandoverModal from "../components/inventory/InventoryHandoverModal";
import { Loader2, AlertCircle } from "lucide-react";

export default function InventoryHandover() {
  ...
  const fetchRequisition = async () => {
    try {
      ...
      // Check if requisition is in the correct status for inventory handover
      if (requisitionData.status !== "Issued") {
        setError("This requisition is not ready for inventory handover. Current status: " + requisitionData.status);
        return;
      }
